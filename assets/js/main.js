 ...
 */
(function initTheme() {
  // Get the storage key from data attribute or use default
  const STORAGE_KEY = document.documentElement.dataset.themeKey || 'docstone-theme';
...
function initThemeSwitcher() {
  ...
  options.forEach((option) => {
    option.addEventListener('click', () => {
      ...
      window.docstoneTheme.set(theme);
      updateActiveOption();
      menu.classList.remove('active');
      toggle.setAttribute('aria-expanded', 'false');
    });
  ...
  function updateActiveOption() {
    const currentTheme = window.docstoneTheme.get();
    options.forEach((option) => {
